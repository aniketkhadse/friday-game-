...
function effectiveState(now = Date.now()) {
  if (game.gameState === "COUNTDOWN" && game.countdownEndsAt && now >= game.countdownEndsAt) {
  ...
  if (game.gameState === "STARTED" && game.roundEndsAt && now >= game.roundEndsAt) {
